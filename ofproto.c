    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    char *trans_data = ofpacts.data;
    //for analyze
    //for(int i=0;i<ofpacts.size;i++){
    //  printf("%x ", (unsigned) *(trans_data+i));
    //}
    //printf("\n");
    ...
    if (!error) {
        ...
        //only relate to run_rule_executes() when out_port is controller
		if ((unsigned) *(trans_data+4) == 0xfffffffd) {
